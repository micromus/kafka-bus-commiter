# Changelog

All notable changes to `kafka-bus-repeater` will be documented in this file.

## v1.1.2 - 2026-06-05

### What's Changed

* add: Добавлены тесты и актуализирована документация by @popkovkirill in https://github.com/micromus/kafka-bus-commiter/pull/5

**Full Changelog**: https://github.com/micromus/kafka-bus-commiter/compare/v1.1.1...v1.1.2

## v1.1.1 - 2026-06-04

### What's Changed

* add: Добавлены тесты и актуализирована документация by @popkovkirill in https://github.com/micromus/kafka-bus-commiter/pull/4

**Full Changelog**: https://github.com/micromus/kafka-bus-commiter/compare/v1.1.0...v1.1.1

## v1.1.0 - 2026-06-04

### What's Changed

* refactor: Переработан механизм защиты от двойного чтения и введен клю… by @popkovkirill in https://github.com/micromus/kafka-bus-commiter/pull/3

**Full Changelog**: https://github.com/micromus/kafka-bus-commiter/compare/v1.0.0...v1.1.0

## v1.0.0 - 2026-05-27

### What's Changed

* refactor: Обновление ядра Kafka Bus by @popkovkirill in https://github.com/micromus/kafka-bus-commiter/pull/1
* feat: Пополнена документация в Readme by @popkovkirill in https://github.com/micromus/kafka-bus-commiter/pull/2

### New Contributors

* @popkovkirill made their first contribution in https://github.com/micromus/kafka-bus-commiter/pull/1

**Full Changelog**: https://github.com/micromus/kafka-bus-commiter/commits/v1.0.0

## v0.3.2 - 2025-01-06

**Full Changelog**: https://github.com/micromus/kafka-bus-repeater/compare/v0.3.1...v0.3.2

## v0.3.1 - 2024-12-15

**Full Changelog**: https://github.com/micromus/kafka-bus-repeater/compare/v0.3.0...v0.3.1

## v0.3.0 - 2024-12-15

**Full Changelog**: https://github.com/micromus/kafka-bus-repeater/compare/v0.2.0...v0.3.0

## v0.2.0 - 2024-12-14

**Full Changelog**: https://github.com/micromus/kafka-bus-repeater/compare/v0.1.1...v0.2.0

## v0.1.1 - 2024-11-29

**Full Changelog**: https://github.com/micromus/kafka-bus-repeater/compare/v0.1.0...v0.1.1

## v0.1.0 - 2024-11-29

- MVP
